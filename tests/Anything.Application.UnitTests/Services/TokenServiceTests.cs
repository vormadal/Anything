...
public class TokenServiceTests
{
    ...
    {
        var principal = CreateService().GetPrincipalFromToken("invalid.token.here");

        Assert.Null(principal);
    }

    [Fact]
    public void GetPrincipalFromToken_TokenWithWrongSigningKey_ReturnsNull()
    {
        _timeProvider.GetUtcNow().Returns(new DateTimeOffset(2026, 3, 10, 12, 0, 0, TimeSpan.Zero));
        var differentSettings = new JwtSettings
        {
            SecretKey = "completely-different-secret-key-32-chars",
            Issuer = "TestIssuer",
            Audience = "TestAudience",
            AccessTokenExpirationMinutes = 15
        };
    ...
    [Fact]
    public void GetPrincipalFromToken_ExpiredToken_StillReturnsPrincipal()
    {
