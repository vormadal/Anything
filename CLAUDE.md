...
- **API client is auto-generated:** The `update-api-client` workflow boots the API (Postgres + Minio, in the `Development` environment so Swagger is exposed) and runs `npm run generate:api`, committing the regenerated `anything-frontend/src/lib/api-client/**` back to the branch on pushes that touch the backend API surface (`src/Anything.API/**`, `src/Anything.Contracts/**`, `src/Anything.Application/**`, `src/Anything.Core/**`). **Do not hand-edit the generated client.** Until it regenerates, new endpoints can be called with `apiFetch` from `@/lib/apiClient` (the authenticated raw-fetch helper for endpoints not yet in the Kiota client). Note: the `kiota` binary is a .NET tool (`Microsoft.OpenApi.Kiota`), not an npm package.
