...
Security/performance rules live in the layer `agent.md`s above (API, Application, Contracts, Database, frontend lib/hooks). The 2026-08 remediation plan that produced them (`SECURITY-PERFORMANCE-PLAN.md`) has been deleted now that every item has landed — its durable "why" is in those files, not a separate tracker.
...
- **Failed loads are not empty states.** Any place that lists data must ask `loadFailure(...)` (`@/lib/queryState`) whether the query failed — or never ran because the browser is offline — and render `LoadErrorState` instead of "No X yet". Without it a connectivity problem is indistinguishable from an empty household. Full rules in [`anything-frontend/src/components/agent.md`](anything-frontend/src/components/agent.md).
...
- **None of `frontend-ci.yml`/`backend-ci.yml`/the three bot workflows accept `workflow_dispatch`, so a stalled check can't be manually re-fired via the API.** They only listen for `push`/`pull_request` (the bot workflows additionally gate on `paths:` — see the sync-commit gotcha above). `actions_run_trigger`'s `run_workflow` method (a `POST .../actions/workflows/<file>/dispatches` call) returns `403 Resource not accessible by integration` against all of them for exactly this reason — it's not a permissions issue to fix, the event type genuinely isn't wired up. The only way to get a fresh check run on an existing commit is a new commit that touches one of the workflow's trigger paths (a real one — see CLAUDE.md's "Record expensive learnings" — not an empty commit); if GitHub Actions itself is stalled repo-wide (confirm via `actions_list` with no branch filter showing no runs anywhere, not just this branch), no push will make a check appear until Actions resumes, and that's on GitHub, not the repo.
...
- **Changing a global UI element (the app header, say) makes only *some* baselines stale — find out which with `git diff --name-only <commit-that-added-it> HEAD -- anything-frontend/e2e/visual.spec.ts-snapshots/`.** Because `--update-snapshots=changed` skips anything under the 2% threshold (previous gotcha), an element that first shipped under that threshold was never written into the baselines that already existed — so most PNGs show the app as it was *before* that element, and removing it again brings them back into agreement rather than invalidating them. Concretely: `NotificationBell` was added to `AppLayout`'s header in `3ed3fd0`, and only **16 of 152** baselines were regenerated afterwards; those 16 are the only ones carrying a bell, so removing it needed exactly those 16 `git rm`-ed (paired with a trigger-path change, per the previous gotcha) and left the other 136 correct. Deleting all ~150 "to be safe" would have been a huge, mostly no-op PNG diff. The git-log question answers in one command what reading pixels cannot.
  - **Don't try to establish the affected set by running the visual suite locally at a near-zero `maxDiffPixels`.** The suite is `fullyParallel` with an unpinned worker count, and full-suite runs are not reproducible at that sensitivity — the same pre-change build produced 115 failures in one full run and, for the subset it claimed had passed, 9/11 failures when those tests were re-run alone. Local runs are for verifying *locators*, as `.claude/rules/e2e.md` says; they cannot tell you which baselines a change invalidates.
...
- **Queries run on `/login` too, and 4xx isn't retried — anything that must work right after login needs refetching on login.** `HouseholdProvider` sits in the root layout, so `useHouseholds()` fires on `/login` with no token and 401s. `QueryProvider`'s `retry` fails 4xx fast (added in #755); before that, React Query's default backoff retries happened to land after login and hid the problem. `router.push("/")` keeps the provider mounted, so the failed query never re-ran, `householdId` was never written, and the deploy's e2e `global.setup.ts` timed out on `waitForFunction(localStorage.getItem("householdId"))` (real users were also left without a selected household until reload). `useLogin`'s `onSuccess` now calls `queryClient.invalidateQueries()` with no filter. Don't narrow it back to `["auth","user"]`: everything cached before login was fetched anonymously.
