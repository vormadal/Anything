...
- **`EF.Functions.ToTsVector`/`PlainToTsQuery`/`ToTsQuery` and `.Matches(...)` are natively supported** by the Npgsql EF provider (unlike `word_similarity`/`similarity` — no `HasDbFunction` stub needed for these).
...
- **Backfill is an explicit admin command** (`RebuildSearchIndexCommand`, `POST /api/search/rebuild-index`), not automatic on startup — run once after this feature deploys, or after changing what gets indexed.
