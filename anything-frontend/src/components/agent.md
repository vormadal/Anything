...
- **Checked items sort most-recently-checked first**, not by their original position, so undoing a misclick is a tap on the top row instead of a hunt through the list. `sortMostRecentlyCheckedFirst`/`latestDate` in `@/lib/checklistOrder` order by `modifiedOn` (touched on every check/uncheck server-side), which survives a refetch or SSE sync — a purely local "just-toggled" ordering wouldn't. Applied in `GeneralChecklistView`, both of `ShoppingListView`'s sort modes (aggregating first means an `AggregatedItem` needs its own `modifiedOn`, taken as the latest across its underlying rows), and the note embed's `EmbeddedListItems`. The reorder is animated via `useFlipAnimation` (`@/hooks/agent.md`) — every animatable `<li>` needs a `data-flip-id`.
