...
interface InventoryAttachmentsProps {
...
/**
 * Shared photos + documents section for items, boxes and places — each owner
 * gets its own attachment hooks (list/upload/download/delete), but the UI is
 * identical, so pages pass the hook results in rather than this component
 * knowing which owner it's attached to.
 */
export function InventoryAttachments({
  ...
  const photos = (attachments ?? []).filter((a) => a.kind === InventoryAttachmentKinds.Photo);
          ...
          <div className="flex gap-2 flex-wrap">
            {photos.map((photo) => (
              <div key={photo.id} className="relative h-20 w-20">
                ...
                <button
                  type="button"
                  onClick={() => handleDelete(photo.id ?? 0)}
                  disabled={isDeleting || !isOnline}
                  title={isOnline ? undefined : OFFLINE_HINT}
                  aria-label="Remove photo"
                  className="absolute -top-1.5 -right-1.5 h-5 w-5 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center disabled:opacity-50"
                >
