...
- **Reads**: `persister.ts` persists the *entire* React Query cache to IndexedDB via `@tanstack/query-persist-client-core`/`@tanstack/react-query-persist-client`, denylisted by `shouldPersistQuery` to exclude only `auth`-prefixed query keys (`auth.user` reads straight from `localStorage` already; invite tokens are time-sensitive). This is what lets a cold, offline app open still render the last-loaded household/recipes/food-plan/bills/etc. instead of a blank screen (issue #622) — new query keys are persisted automatically, no opt-in needed. `QueryProvider` wires this up via `PersistQueryClientProvider`, and `AuthGuard` holds its loading state on `useIsRestoring()` so children never mount before the persisted cache has rehydrated.
  - **`createAsyncStoragePersister` defaults to plain `JSON.stringify`/`JSON.parse`, which silently turns every `Date` field (`modifiedOn`, `createdOn`, `purchasedOn`, ...) on every persisted entity into a plain string.** Code that assumes a real `Date` (e.g. `checklistOrder.ts`'s `sortMostRecentlyCheckedFirst` calling `item.modifiedOn?.getTime()`) throws `getTime is not a function` the moment it renders against a just-rehydrated cache — visible only after a reload, before `onSuccess`'s `invalidateQueries()` refetch lands. No test caught this because every test builds items with real `Date` objects directly; nothing exercises the JSON round-trip. Fixed by giving the persister explicit `serialize`/`deserialize` options (`serializeOfflineCache`/`deserializeOfflineCache`) that mark/unmark `Date` instances before/after the `JSON.stringify`/`parse` call. **This can't be done with a `JSON.stringify` replacer checking `instanceof Date`** — `JSON.stringify` calls `Date.prototype.toJSON()` (already producing the ISO string) *before* the replacer ever sees the value, so by then it's indistinguishable from any other string. The fix instead walks the object tree itself first (`markDates`/`unmarkDates`), tagging real `Date` instances only — never based on string shape, so an ordinary string that happens to look like an ISO date is left alone.
